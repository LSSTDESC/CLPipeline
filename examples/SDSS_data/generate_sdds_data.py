...
def generate_SDSSCL_sacc_file() -> Any:
    ...
    s_count.save_fits("./outputs/clusters_SDSS_sacc_file_cov.sacc", overwrite=True)
    total_clusters = np.sum(cluster_counts)
    total_mean_logM = np.sum(cluster_counts * mean_logM) / total_clusters
    print(f"Total mean logM: {total_mean_logM}, {np.log(10**total_mean_logM)} ")
